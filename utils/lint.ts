import {
  ...
  PanelConstructor, Panel, showPanel, getPanel,
} from "@codemirror/view";
import {
  Text, StateEffect, StateField, Extension, TransactionSpec, Transaction,
  EditorState, Facet, combineConfig, RangeSet, Range,
...
class LintState {
  ...
  static init(diagnostics: readonly Diagnostic[], panel: PanelConstructor | null, state: EditorState) {
    ...
    const ranges = Decoration.set(markedDiagnostics.map((d: Diagnostic) => {
...
function batchResults<T>(promises: readonly Promise<T>[], sink: (values: T[]) => void, error: (reason: any) => void) {
  const collected: T[] = [], timeout = -1;
  for (const p of promises) p.then(value => {
...
function assignKeys(actions: readonly Action[] | undefined) {
  ...
  if (actions) actions: for (const { name } of actions) {
...
function renderDiagnostic(view: EditorView, diagnostic: Diagnostic, inPanel: boolean) {
  ...
  return elt(
    ...
    diagnostic.actions?.map((action, i) => {
      ...
      const nameElt = keyIndex < 0 ? name : [name.slice(0, keyIndex),
...
function markersForDiagnostics(doc: Text, diagnostics: readonly Diagnostic[]) {
  const byLine: Record<number, Diagnostic[]> = Object.create(null);
...
const lintGutterTheme = EditorView.baseTheme({
  ...
  ".cm-lint-marker-error": {
    content: svg(`<circle cx="16" cy="16" r="12" fill="#f87" stroke="#f43" stroke-width="6"/>`),
  },
});
