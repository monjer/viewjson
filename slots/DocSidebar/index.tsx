...
function ItemLink({ href, title, className = "", style = {} }: ItemLink) {
  ...
  return (
    <Link href={href} title={title} className={`no-underline hover:text-black  dark:hover:text-white ${className} ${active ? 'text-black font-semibold dark:text-white' : ''}`} style={style}>{title}</Link>
  );
}
...
function NavItem({ item }: { item: NavItemProps }) {
  ...
  const className = active ? ' border-l-gray-900 dark:text-gray-100 dark:border-l-gray-400' : ' border-l-stone-300 dark:border-l-gray-700';
  return (
...
function SubNav({ item }: { item: NavItemProps }) {
  ...
  return (
    <>
      <h1 className="text-sm m-0">{href ? <ItemLink href={href} title={title} /> : title}</h1>
